...
export let hallFrag = "precision mediump float;uniform ivec4 g;uniform int f;uniform float h;varying vec2 c;float l(float a){return 1.-a*a;}void main(){vec4 b=vec4(0,0,0,1),d=vec4(1,1,1,1);d=f==0?vec4(0,.5,0,1):f==1?mod(c.x*10.,2.)>1.&&mod(c.y*10.,2.)>1.||mod(c.x*10.,2.)<1.&&mod(c.y*10.,2.)<1.?vec4(1,1,1,1):vec4(0,0,0,1):f==2?vec4(.5,.5,.5,1):d;vec4 a=mix(vec4(.5,.5,.5,1),d,1.-l(f<2?1.:h));a=g.x==0&&c.x<.01?b:a,a=g.z==0&&c.x>.99?b:a,a=g.y==0&&c.y<.01?b:a,a=g.w==0&&c.y>.99?b:a,gl_FragColor=a;}";
